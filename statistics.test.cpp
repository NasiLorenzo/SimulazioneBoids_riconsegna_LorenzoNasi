...
TEST_CASE("Testig Sample class")
{
  ...
  SUBCASE("Testing after addition"){
    ...
    CHECK(test_sample.result().sigma == doctest::Approx(0.7637).epsilon(0.0001));
  }
...
TEST_CASE("Testing statistics of a flock"){
    ...
    CHECK(flock_data.pos_stats[0].result().sigma==doctest::Approx(1.118).epsilon(0.001));

    CHECK(flock_data.pos_stats[1].result().mean==doctest::Approx(2.75));
    CHECK(flock_data.pos_stats[1].result().sigma==doctest::Approx(1.479).epsilon(0.001));

    CHECK(flock_data.vel_stats[0].result().mean==doctest::Approx(2.5));
    CHECK(flock_data.vel_stats[0].result().sigma==doctest::Approx(1.118).epsilon(0.001));

    CHECK(flock_data.vel_stats[1].result().mean==doctest::Approx(3.5));
    CHECK(flock_data.vel_stats[1].result().sigma==doctest::Approx(1.802).epsilon(0.001));

    CHECK(flock_data.pos_mod_stats.result().mean==doctest::Approx(3.168).epsilon(0.001));
    CHECK(flock_data.pos_mod_stats.result().sigma==doctest::Approx(1.792).epsilon(0.001));

    CHECK(flock_data.vel_mod_stats.result().mean==doctest::Approx(4.307).epsilon(0.001));
    CHECK(flock_data.vel_mod_stats.result().sigma==doctest::Approx(2.101).epsilon(0.01));

    CHECK(flock_data.distance_stats.result().mean==doctest::Approx(2.749).epsilon(0.001));
    CHECK(flock_data.distance_stats.result().sigma==doctest::Approx(1.267).epsilon(0.001));
