    ...
    const int columns)
{
  neighbors.clear();
  gridID startID{};
  if (boid_.set_boid().GridID.columns == 0)
    startID.columns = 0;
  startID.columns = boid_.set_boid().GridID.columns - 1;
  if (boid_.set_boid().GridID.rows == 0)
    startID.rows = 0;
  startID.rows  = boid_.set_boid().GridID.rows - 1;
