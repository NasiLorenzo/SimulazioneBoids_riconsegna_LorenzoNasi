      ...
      std::for_each(neighrange.first, neighrange.second, [&](auto& neighbor) {
        ...
        if (distanza < pow(align_distance, 2) && distanza != 0
          ...
          if ((cosangolo) >= std::cos(alpha)) {
            ...
            /*std::cout << "La posizione  vicino vale: "
                      << neighbor.second->pos_[0] << " e "
                      << neighbor.second->pos_[1] << "\n";*/
          }
        }
      });
    }
    ...
    const double alpha, unsigned int size, unsigned int flocksize, const int x)
{
  if (flocksize < size) {
    update_neighbors(boid_, neighbors, map, align_distance, alpha,
                     Criterion::similar, x);
    update_close_neighbors(boid_, close_neighbors, map, repulsion_distance, x,
                           alpha);
  } else {
    update_neighbors(boid_, neighbors, map, align_distance, alpha,
                     Criterion::any, x);
...
void flock::update(paramlist const& params)
{
  std::for_each(
      std::execution::par_unseq, set.begin(), set.end(), [&](auto& boid) {
        ...
        // std::cout<<"Chiave boid "<<boid.set_GridID().x<<" e
        //"<<boid.set_GridID().y<<"\n";
        /*std::cout << "La chiave vale: "
                  << hash_function(boid.set_GridID(), params.x)
                  << "\n";*/
