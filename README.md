...
L'hasher custom gridID_hash così creato viene aggiunto come parametro di template opzionale della mappa. E' inoltre necessario definire esplicitamente un `operator==` per `GridID`, in modo tale che la mappa possa confrontare i valori delle chiavi e gestire potenziali collisioni.
...
La griglia secondo la quale vengono stabiliti i valori delle chiavi è quadrata, di lato corrispondente al `view_range` dei boid. In questo modo posso avere la certezza che i potenziali vicini di un boid si trovino in un range di sottospazi della griglia che hanno coordinate comprese tra -1 e +1 rispetto a quelle del boid di riferimento. 
